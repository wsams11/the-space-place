const astronomyList = {
  ...
  'nebula': ['H II regions', 'Planetary nebula', 'Supernova remnant', 'Dark nebula'],
};

for (item in astronomyList) {
  $('#astronomyList').append(
    $('<p>').addClass("menu-label").text(item[0].toUpperCase() + item.slice(1).toLowerCase())
  );

  astronomyList[item].forEach(function (type) {
    $('<ul>').addClass("menu-list").append(
      $('<li>').append($("<a>")).text(type)
